...
document.addEventListener('DOMContentLoaded', preload);
  ...
  class Environment {
	...
	createBackground() {
    // Function to update background size and position
    const updateBackgroundSizeAndPosition = () => {
        // Calculate aspect ratio
        const aspectRatio = window.innerWidth / window.innerHeight;

        // Adjust background size based on aspect ratio
        const width = 2000 * aspectRatio;
        const height = 2000;

        // Update background geometry size
        this.backgroundMesh.geometry = new THREE.PlaneGeometry(width, height);

        // Update background mesh position
        this.backgroundMesh.position.z = -1500;
    };
	...
	const backgroundTexture = new THREE.TextureLoader().load('code.jpg', function (texture) {
		...
		texture.magFilter = THREE.LinearFilter; // or other options like THREE.NearestFilter
		texture.minFilter = THREE.LinearMipMapLinearFilter; // or other options
		texture.anisotropy = 15; // Experiment with different values
	});
    ...
    // Add background mesh to the scene
    this.scene.add(this.backgroundMesh);

    // Initial background setup
    updateBackgroundSizeAndPosition();

    // Listen for window resize events to update background size and position
    window.addEventListener('resize', updateBackgroundSizeAndPosition);
}
